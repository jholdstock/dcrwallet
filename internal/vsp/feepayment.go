...
func (fp *feePayment) reconcilePayment() error {
	...
	if feeTx == nil || len(feeTx.TxOut) == 0 {
		...
		if err != nil {
			...
			if errors.As(err, &apiErr) && apiErr.Code == int64(types.ErrTicketCannotVote) {
	...
	if errors.As(err, &apiErr) {
		switch apiErr.Code {
		case int64(types.ErrFeeAlreadyReceived):
			...
			err = nil
		case int64(types.ErrInvalidFeeTx), int64(types.ErrCannotBroadcastFee):
...
func (fp *feePayment) submitPayment() (err error) {
	...
	if err != nil {
		var apiErr types.ErrorResponse
		if errors.As(err, &apiErr) && apiErr.Code == int64(types.ErrFeeExpired) {
